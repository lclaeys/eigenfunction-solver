...
class DoubleWellHard(NeuralSDE):
    def __init__(
        self,
        device="cuda",
        dim=2,
        ...
        self.kappa = kappa
        self.nu = nu
        self.nu_matrix = torch.diag(self.nu)
        self.confining=True
    ...
    # Running cost
    def f(self, t, x):
        x_dist = x - torch.ones_like(x,device=x.device)
        return torch.sum(
            x_dist * torch.einsum("ij,...j->...i", self.nu_matrix, x_dist), -1
        )

    # Gradient of running cost
    def nabla_f(self, t, x):
        x_dist = x - torch.ones_like(x,device=x.device)
        return 2 * torch.einsum("ij,...j->...i", self.nu_matrix, x_dist)
    ...
    def scalar_f(self, x, idx,cpu=False):
        if cpu:
            return self.nu.cpu().numpy()[idx] * (x-1.0)**2
        else:
            return self.nu[idx] * (x-1.0)**2
